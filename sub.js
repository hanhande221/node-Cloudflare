...
export default {
  async fetch(request, env) {
    ...
    if (path === "/sub") return handleSub(env);
...
function handleAdmin(origin) {
  ...
  const script = [
    ...
    // 实时预览 Subscription-Userinfo header 内容
    ...
    "  if(!dateStr){",
    "    bar.className='expiry-bar expiry-none';",
    "    bar.textContent='📅 未设置到期时间';",
    "    return;",
    "  }",
    ...
    "      loadNodes();loadExpiry();loadTraffic();",
    ...
    "    var d=await r.json();",
    "    document.getElementById('ta').value=d.nodes.join('\\n');",
    "    stat();",
    ...
    "  try{",
    "    var r=await fetch('/api/getexpiry',{credentials:'include'});",
    "    var d=await r.json();",
    "    var val=d.expiry||'';",
    "    document.getElementById('expiry-input').value=val;",
    "    renderExpiryBar(val);updatePreview();",
    "  }catch(e){}",
    "}",
    ...
    "  try{",
    "    var r=await fetch('/api/setexpiry',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({expiry:val})});",
    "    var d=await r.json();",
    "    if(d.ok){toast('到期时间已保存 ✓');renderExpiryBar(val);updatePreview();}",
    ...
    "  try{",
    "    var r=await fetch('/api/gettraffic',{credentials:'include'});",
    "    var d=await r.json();",
    "    var t=d.traffic||{};",
    "    document.getElementById('t-up').value=t.upload||'';",
    "    document.getElementById('t-dl').value=t.download||'';",
    "    document.getElementById('t-tot').value=t.total||'';",
    "    updatePreview();",
    "  }catch(e){}",
    "}",
    ...
    "  var traffic={",
    "    upload:document.getElementById('t-up').value,",
    "    download:document.getElementById('t-dl').value,",
    "    total:document.getElementById('t-tot').value",
    "  };",
    "  try{",
    "    var r=await fetch('/api/settraffic',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({traffic:traffic})});",
    "    var d=await r.json();",
    "    if(d.ok)toast('流量信息已保存 ✓');",
    "    else toast('保存失败',true);",
    "  }catch(e){toast('保存失败',true);}",
    "}",

    "function copyUrl(){",
    "  navigator.clipboard.writeText('" + subUrl + "').then(function(){toast('已复制 ✓');});",
    "}",
